...
class UsuarioController
{
    public function login()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $erro = '';
            if (isset($_GET['erro']) && $_GET['erro'] === 'usuario_inexistente') {
            $erro = 'Seu usuário não existe mais. Faça login novamente com outro usuário.';
        }
    ...
    public function excluir()
    {
        ...
        if (Usuario::deletar($_POST['id'])) {
            if (isset($_SESSION['usuario']) && $_SESSION['usuario']['id'] == $_POST['id']) {
                unset($_SESSION['usuario']);
                $_SESSION['erro'] = 'Sua conta foi excluída. Você foi desconectado.';
                header('Location: ?pagina=usuario&acao=login');
                exit;
            }
            ...
            $usuario = Usuario::buscarPorId($_GET['id']);
            $token = Seguranca::gerarToken();
            $conteudo = '../app/view/usuario/confirmar_exclusao.php'; 
            include '../app/view/layout.php';
        }
    }
